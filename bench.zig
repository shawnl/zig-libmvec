...
extern fn _ZGVcN4v_exp(x: V4) V4;
const four = _ZGVcN4v_exp;
extern fn _ZGVdN4v_exp(x: V4) V4;
const four_inaccurate = _ZGVdN4v_exp;
...
var four_sys_inaccurate: f64 = undefined; // avx2 path
...
}
{
    ...
    for (s) |pair, j| {
        res[j] = four_inaccurate(pair);//exp(V4, pair);
    }
    end = timer.read();
try check(resbuf);
four_sys_inaccurate = @intToFloat(f64, end - start);
...
//    const throughput = @floatToInt(u64, @intToFloat(f64, buf.len) / elapsed_s);
    ...
    try stdout.print("4 avx2: {} bytes/s {} bytes/s ({}%)\n", len / four_sys_inaccurate, len /four_native, four_sys_inaccurate / four_native);
